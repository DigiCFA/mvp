  ...
  return (
    <SafeAreaView className="bg-[#e9e7e2]">
      ...
      <View className="flex-row justify-items-start items-center space-x-2 pt-1 mx-4">
        ...
        <Text className="text-lg font-semibold">{card.cardType.charAt(0).toUpperCase() + card.cardType.slice(1)} ••••{card.cardNumber.slice(-4)}</Text>
      ...
      <View className='bg-white p-4'>
        <View className='flex-col'>
            ...
            <Text className='text-black text-lg font-semibold'>{card.expDate.substring(5, 7)}/{card.expDate.substring(2, 4)}</Text>

            <Text className='text-gray-600 pt-8'>Billing address</Text>
            <Text className='text-black text-lg font-semibold'>{card.billingAddress}</Text>

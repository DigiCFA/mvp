...
const SetProfileScreen = () => {
    ...
    return (
        <SafeAreaView className="bg-white flex-1">
            ...
            <HideKeyboardView>
                <View className="mx-3">
                    <Text className="font-semibold" style={{fontSize: 30}} >Set up your profile</Text>
                    <Text className="font-medium" style={{fontSize: 18}}>This information needs to be accurate</Text>
                </View>
            </HideKeyboardView>

            <View className="mx-3 mt-10 space-y-6">
                <TextInput placeholder='First name' style={{fontSize: 18}}
                    className={`border px-3 py-5 rounded-md ${firstNameInputFocused ? 'border-blue-500' : 'border-gray-500'}`}  
                    onBlur={() => {setFirstNameInputFocused(false)}}
                    onFocus={() => {setFirstNameInputFocused(true)}}
                    />

                <TextInput placeholder='Last name' style={{fontSize: 18}}
                    className={`border px-3 py-5 rounded-md ${lastNameInputFocused ? 'border-blue-500' : 'border-gray-500'}`}
